...
import time
import copy
from global_config import DEVICE, IMAGE_SIZE, MODEL_SAVE_FILE
...
def train_model(data_loader, model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ["train", "val"]:
            ...
            for inputs, labels in data_loader.load_data(data_set=phase):
...
def train():
    data_loader = DataLoader(
        data_dir="datasets/hymenoptera_data", image_size=IMAGE_SIZE, batch_size=4
    )
    ...
    try:
        model = train_model(
            data_loader, model, criterion, optimizer_ft, exp_lr_scheduler, num_epochs=25
        )
